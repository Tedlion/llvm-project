//=== ExpansionAssociatedRange.cpp - AssociatedRange with Macro Expansion --===/
//
...
//===-----------------------------------------------------------------------===/
//
// This file implements getExpansionAssociatedRange almost the same with
// getAssociatedRange. The difference is when the BeginLoc or EndLoc is inside a
// macro expansion, we will expand the range to include the whole macro. Notice:
// other AST units expanded from the macro may be included in the range.
//
//===----------------------------------------------------------------------===//
...
static std::set<tok::TokenKind> getTerminators(const Decl &D) {
  if (llvm::isa<RecordDecl>(D) || llvm::isa<UsingDecl>(D) ||
      llvm::isa<TypedefDecl>(D) || llvm::isa<EnumDecl>(D))
    return {tok::semi};
